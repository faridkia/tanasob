...
- **[F1]** ساخت پروژه Vite + React + axios + react-router + Tailwind؛ AuthContext
- **[F2]** صفحات ورود/ثبت‌نام + ذخیره JWT
- **[F3]** لایوت داشبورد + routing بر اساس نقش + هدر/سایدبار
- **[F4]** داشبورد و صفحات Member: خانه، خرید اشتراک، فهرست کلاس‌ها + رزرو، برنامه‌ها (تمرین/رژیم)، پیشرفت بدن + چارت، چت، اعلان‌ها
- **[F5]** داشبورد Trainer: جلسات من، اعضای من، ساخت/ویرایش برنامه، چت، ثبت حضور غیاب
- **[F6]** داشبورد Admin: کاربران، پلن‌ها، کلاس‌ها/جلسات، گزارش‌ها، پرداخت‌ها
- **[F7]** پولیش نهایی + ریسپانسیو + تست GUI
...
## 📊 وضعیت فعلی
- **در حال اجرا:** فاز ۱ (پیاده‌سازی بک‌اند کامل است؛ تست‌های یکپارچه نهایی باقی مانده‌اند)
- **تسک جاری:** T16 — تکمیل تست‌های نقش‌ها و endpointها، سپس شروع F1
